...
export default function FinancialAnalystLayout({ children }: { children: ReactNode }) {
  ...
  const navItems = [
    { name: 'Dashboard', href: '/financial-analyst' },
    { name: 'Fuel Logs', href: '/financial-analyst/fuel-logs' },
    { name: 'Expenses', href: '/financial-analyst/expenses' },
    { name: 'Analytics', href: '/financial-analyst/analytics' },
  ];
    ...
    <div className="flex h-screen bg-[#111111] text-zinc-100 overflow-hidden font-sans">
      ...
      <aside className="w-64 border-r border-zinc-800 flex flex-col">
        ...
        <nav className="flex-1 px-4 py-4 space-y-2">
          {navItems.map((item) => {
            const isActive = pathname === item.href || (item.href !== '/financial-analyst' && pathname.startsWith(item.href));
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`block px-4 py-2.5 rounded-lg transition-colors ${
                  isActive
                    ? 'border border-green-500/50 bg-green-500/10 text-green-400 font-medium'
        ...
        <header className="h-16 border-b border-zinc-800 flex items-center justify-between px-8 bg-[#111111]">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs font-medium text-green-400 border border-green-400/30 bg-green-400/10 px-2 py-1 rounded">
                {user?.role === 'FINANCIAL_ANALYST' ? 'Financial Analyst' : user?.role || '...'}
              </span>
