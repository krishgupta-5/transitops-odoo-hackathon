...
import { Sidebar, DEFAULT_FINANCIAL_NAV_ITEMS } from "@/components/Sidebar";
...
export default function FinancialAnalystLayout({ children }: { children: ReactNode }) {
  ...
  const displayName = user?.name || 'Demo Financial Analyst';
  const avatarInitials = user?.name ? user.name.substring(0, 2).toUpperCase() : 'FA';
  ...
  return (
    <div className="flex h-screen bg-[#F4F4F5] dark:bg-[#070707] text-gray-900 dark:text-gray-100 overflow-hidden font-sans p-3 gap-3 transition-colors">
      ...
      <Sidebar
        navItems={DEFAULT_FINANCIAL_NAV_ITEMS}
        roleName="Financial Analyst"
        profileHref="/financial-analyst/profile"
        user={user}
      />
